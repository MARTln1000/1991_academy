...
def calculate_entropy(y):
  ...
  # YOUR CODE HERE
    ...
    N = np.unique(y, return_counts=True)[1]
    if len(N) == 1:
        E = 0
    else:
        p = N[1] / (N[1] + N[0])
        E = -1 * p * np.log2(p) - (1 - p) * np.log2(1 - p)
    entropy = E
    return entropy
...
class DTClassifier:
  ...
  def calculate_purity_gain(self, y, y1, y2):
    ...
    # YOUR CODE HERE
        if self.impurity == calculate_entropy:
            pure_gain = calculate_entropy(y) - ((len(y1) * calculate_entropy(y1) + len(y2) * calculate_entropy(y2)) / (len(y1) + len(y2)))
        elif self.impurity == calculate_gini:
            pure_gain = calculate_gini(y) - ((len(y1) * calculate_gini(y1) + len(y2) * calculate_gini(y2)) / (len(y1) + len(y2)))
  ...
  def divide_on_feature(self, X, y, feature_id, threshold):
    ...
    if isinstance(threshold, int) or isinstance(threshold, float):
        true_indices = X[: , feature_id] >= threshold # YOUR CODE HERE 
    else:
        true_indices = X[: , feature_id] == threshold # YOUR CODE HERE
  ...
  def majority_vote(self, y):
    ...
    # YOUR CODE HERE
    k = list(y.reshape(y.shape[0]))
    label = np.argmax([k.count(0),k.count(1)])
    return label
