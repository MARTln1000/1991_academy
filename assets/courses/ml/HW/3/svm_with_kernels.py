...
class SupportVectorMachine:
  ...
  def fit(self, X, y):
    ...
    self.t = self.t - self.support_vector_labels[0]  
  ...
  def predict(self, X):
    ...
    for instance in np.array(X):    
      ...
      for i in range(len(self.alphas)):
          prediction += self.alphas[i] * self.support_vector_labels[i] * self.kernel(self.support_vectors[i], instance)  # YOUR CODE HERE          
      prediction -= self.t
